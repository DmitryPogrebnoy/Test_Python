...
void PyMainAlgWorker::run(double** arguments)
{
    try{
        ...
        QScopedPointer<PyObject, ScopedPointerPyObjectDeleter> py_List_Ball(PyArray_SimpleNewFromData(1,dims_Ball, NPY_FLOAT64, arguments[0]));
        QScopedPointer<PyObject, ScopedPointerPyObjectDeleter> py_List_TeamBlue(PyArray_SimpleNewFromData(1, dims_TeamBlue, NPY_FLOAT64, arguments[1]));
        QScopedPointer<PyObject, ScopedPointerPyObjectDeleter> py_List_TeamYellow(PyArray_SimpleNewFromData(1, dims_TeamYellow, NPY_FLOAT64, arguments[2]));
        QScopedPointer<PyObject, ScopedPointerPyObjectDeleter> py_List_BallisInside(PyArray_SimpleNewFromData(1, dims_BallisInside, NPY_FLOAT64, arguments[3]));


        if (PyTuple_SetItem(py_Args.data(), 0, py_List_Ball.data()) != 0){
            throw CouldNotSetItemTupleException("py_Args",0,"py_List_Ball");
        }
        if (PyTuple_SetItem(py_Args.data(), 1, py_List_TeamBlue.data()) != 0){
            throw CouldNotSetItemTupleException("py_Args",1,"py_List_TeamBlue");
        }
        if (PyTuple_SetItem(py_Args.data(), 2, py_List_TeamYellow.data()) != 0){
            throw CouldNotSetItemTupleException("py_Args",2,"py_List_TeamYellow");
        }
        if (PyTuple_SetItem(py_Args.data(), 3, py_List_BallisInside.data()) != 0){
        ...
        py_List_Ball.reset(PyTuple_GetItem(py_Value.data(),0));
        py_List_TeamBlue.reset(PyTuple_GetItem(py_Value.data(),1));
        py_List_TeamYellow.reset(PyTuple_GetItem(py_Value.data(),2));
        py_List_BallisInside.reset(PyTuple_GetItem(py_Value.data(),3));


        if (!PyArray_Check(py_List_Ball.data())){
            throw NotArrayException("py_List_Ball");
        }
        if (!PyArray_Check(py_List_TeamBlue.data())){
            throw NotArrayException("py_List_TeamBlue");
        }
        if (!PyArray_Check(py_List_TeamYellow.data())){
            throw NotArrayException("py_List_TeamYellow");
        }
        if (!PyArray_Check(py_List_BallisInside.data())){
            throw NotArrayException("py_List_BallisInside");
        }


        if (PyArray_NDIM(py_List_Ball.data()) != 1){
            throw WrongDimArrayException("py_List_Ball",1,PyArray_NDIM(py_List_Ball.data()));
        }
        if (PyArray_NDIM(py_List_TeamBlue.data()) != 1){
            throw WrongDimArrayException("py_List_TeamBlue",1,PyArray_NDIM(py_List_TeamBlue.data()));
        }
        if (PyArray_NDIM(py_List_TeamYellow.data()) != 1){
            throw WrongDimArrayException("py_List_TeamYellow",1,PyArray_NDIM(py_List_TeamYellow.data()));
        }
        if (PyArray_NDIM(py_List_BallisInside.data()) != 1){
            throw WrongDimArrayException("py_List_BallisInside",1,PyArray_NDIM(py_List_BallisInside.data()));
        }


        if (PyArray_DIMS(py_List_Ball.data())[0] != 3){
            throw WrongSizeArrayException("py_List_Ball",3,PyArray_DIMS(py_List_Ball.data())[0]);
        }
        if (PyArray_DIMS(py_List_TeamBlue.data())[0] != 48){
            throw WrongSizeArrayException("py_List_TeamBlue",48,PyArray_DIMS(py_List_TeamBlue.data())[0]);
        }
        if (PyArray_DIMS(py_List_TeamYellow.data())[0] != 48){
            throw WrongSizeArrayException("py_List_TeamYellow",48,PyArray_DIMS(py_List_TeamYellow.data())[0]);
        }
        if (PyArray_DIMS(py_List_BallisInside.data())[0] != 1){
            throw WrongSizeArrayException("py_List_BallisInside",1,PyArray_DIMS(py_List_BallisInside.data())[0]);
        }

        double* return_list_Ball = reinterpret_cast<double*>PyArray_DATA(py_List_Ball.data());
        double* return_list_TeamBlue = reinterpret_cast<double*>PyArray_DATA(py_List_TeamBlue.data());
        double* return_list_TeamYellow = reinterpret_cast<double*>PyArray_DATA(py_List_TeamYellow.data());
        double* return_list_BallisInside = reinterpret_cast<double*>PyArray_DATA(py_List_BallisInside.data());
